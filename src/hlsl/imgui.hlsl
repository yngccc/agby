...
vs_output vertex_shader(vs_input vs_input) {
	vs_output output;
	output.position.x = (vs_input.position.x / swap_chain_width) * 2 - 1;
	output.position.y = -((vs_input.position.y / swap_chain_height) * 2 - 1);
	output.position.zw = float2(0, 1);
  output.texcoord = vs_input.texcoord;
	output.color = vs_input.color;
	output.color.xyz = pow(abs(output.color.xyz), float3(2.2f, 2.2f, 2.2f));
	return output;
}
