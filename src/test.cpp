...
int main(int argc, char **argv) {
	...
	m_test(ring_buffer) {
		m_case(double_elems) {
			ring_buffer<uint32> rb = {};
			rb.capacity = 2;
			rb.buffer = new uint32[rb.capacity]();
			uint32 value1 = 10;
			uint32 value2 = 100;
			ring_buffer_write(&rb, value1);
			m_assert(rb.size == 1);
			m_assert(rb.buffer[rb.read_index] == value1);
			ring_buffer_write(&rb, value2);
			m_assert(rb.size == 1);
			m_assert(rb.buffer[rb.read_index] == value2);
			delete[]rb.buffer;
		}
		m_case(many_elems) {
			ring_buffer<uint32> rb = {};
			rb.capacity = 256;
			rb.buffer = new uint32[rb.capacity]();
			for (uint32 i = 0; i < rb.capacity - 1; i += 1) {
				ring_buffer_write(&rb, i);
			}
			m_assert(rb.size == rb.capacity - 1);
			for (uint32 i = 0; i < rb.size; i += 1) {
				m_assert(rb.buffer[(rb.read_index + i) % rb.capacity] == i);
			}
			uint32 value = 9999;
			for (uint32 i = 0; i < 10; i += 1) {
				ring_buffer_write(&rb, value);
			}
			m_assert(rb.read_index == 10);
		}
	}
	m_test(hash_map) {
		m_case(insert) {
	...
	m_test(memory_pool) {
		...
		m_case(allocate_free_allocate_same_block) {
			...
			block = memory_pool_alloc<mat4>(&memory_pool);
			m_assert(block_addr == (uintptr_t)block);
			memory_pool_clear(&memory_pool);
		}
